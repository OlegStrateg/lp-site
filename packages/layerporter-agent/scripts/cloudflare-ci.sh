...
const m=String(h?.error?.message||'').toLowerCase();
if(!(m.includes('schema') || m.includes('response_format') || m.includes('text.format') || m.includes('structured'))) process.exit(24);
NODE
fi
