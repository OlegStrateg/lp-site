...
const codes=new Set(['model_not_found','unsupported_parameter','invalid_request_error','MODEL_HTTP_ERROR','invalid_value','unknown_parameter','invalid_json_schema']);
if(!codes.has(h?.error?.code)) process.exit(21);
NODE
fi
